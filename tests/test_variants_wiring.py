"""{a|b|c} variant groups wired into crispz-krea (prompt_variants + cz_prompt).
...
Covers the family rules for the variant syntax:
  ...
  - <lora:...> tags are read on the EXPANDED text (a LoRA in an unpicked option is
    never applied): protocol (the only entry point that parses them in this fork);
...
def test_cli_txt2img_resolves_seed_and_expands():
    import inspect
    import cz_cli
    assert list(inspect.signature(cz_cli.cli_main).parameters) == ["argv"]
    got = {}
