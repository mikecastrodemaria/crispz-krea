...
> FLUX.1-Krea txt2img + upscaler/detailer studio (a Fooocus-style fork of
...
![crispz-krea — FLUX.1-Krea creation + enhancement studio](assets/screenshot.png)

A standalone FLUX.1-Krea **creation + enhancement** tool, **100% local**, no ComfyUI /
...
- **Models**: one **Z-Image checkpoint** dropdown merging the official base repos
  ...
  bf16 memory footprint, the saving is disk/download only). Still skipped with a clear
  console message: misfiled LoRAs, SVDQuant/Nunchaku INT4, **FLUX.2 Klein / pruned
  variants** (same key names but 5 double blocks vs 19 — detected at the header),
...
- **Presets (Fooocus-style)** (Settings > ⭐ Presets): **save / load / update / delete**
  ...
  models (LoRA/SVDQuant files, Klein variants, foreign GGUFs) get none.
...
- **Asset Browser** (standalone gallery, new tab): opens **instantly** (indexing +
  ...
  **🖼️ icon** next to each **LoRA** dropdown and the **Z-Image checkpoint** dropdown
  ...
  a LoRA whose page gains a Krea 2 or Z-Image release is not an update for
  your FLUX.1-Krea copy.
...
generated pixels. crispz-krea therefore does its own compositing, with four passes
...
misfiled LoRAs, SVDQuant/Nunchaku INT4, FLUX.2 Klein/pruned variants and
foreign-architecture files are refused.
...
- **BF16/FP16, GGUF and ComfyUI FP8/INT8 "scaled" checkpoints all load** (GGUF stays
  quantized in VRAM; FP8/INT8 are dequantized to bf16 in RAM — a 12B FLUX FP8 needs
  ~22 GB). Only misfiled **LoRAs**, **SVDQuant/Nunchaku INT4**, **FLUX.2 Klein/pruned
  variants** and foreign-architecture files are auto-hidden from the list (a
  `checkpoint skipped (...)` line is logged).
...
| Picker | Component | Encoder it takes |
|---|---|---|
| **Text encoder (T5)** | `text_encoder_2`, makes the prompt embeddings | a T5-XXL encoder: `t5`, d_model 4096, 24 layers |
| **Text encoder (CLIP)** | `text_encoder`, makes the pooled embedding | a CLIP-L text model: hidden size 768, 12 layers (a full CLIP folder is read for its text half) |

Each picker is checked against the base repo's own component. The metadata records
`text_encoder_t5` / `text_encoder_clip`. Env `KREA_TEXT_ENCODER_2`, `KREA_TEXT_ENCODER`.
...
the model reloads on the next run. **Refresh encoders** rescans after a download.
