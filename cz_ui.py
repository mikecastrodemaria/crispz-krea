"""crispz-krea - interface Gradio (build_ui) + handlers UI + orchestration commune
...
def _apply_transformer_repo(repo):
    ...
    repo = (repo or "").strip()
    if not repo:
        return ("Transformer override is empty — no change. Pick a model in 'FLUX "
...
def _vram_hint(e):
    ...
    return ("  \n**VRAM saturee**, meme apres vidage du cache : ferme les autres apps GPU "
            ...
            "references. Si le rendu suivant echoue encore, redemarre crispz-krea.")
                ...
                try:
                    ...
                    if "CUDA" in str(e) or "out of memory" in str(e).lower():
                        msg += ("  \n**VRAM saturee** (autre app GPU comme ComfyUI encore chargee ? "
                                ...
                                "crispz-krea** (le contexte CUDA est mort), baisse refine_tile / factor.")
...
def build_ui():
    ...
    with gr.Blocks(title=f"crispz-krea {APP_VERSION}", theme=gr.themes.Default(), css=FOOOCUS_CSS,
                ...
                with gr.Tabs():
                    with gr.Tab("Settings"):
                        ...
                        force_ratio_cb = gr.Radio(
                            ["Off", "Crop to fit", "Extend (outpaint)"],
                            value=("Extend (outpaint)" if (cz_pipeline.FORCE_RATIO and
                                   cz_pipeline.FORCE_RATIO_MODE == "extend")
                            ...
                            info="Crop = the loaded image is centre-cropped to the Aspect ratio "
                                 ...
                                 "Extend = the image is EXTENDED to that ratio instead: the "
                                 "missing bands are outpainted (nothing is lost). "
